...
function Ajax(task, data, form, callback){
	this.task = task;
    this.data = data;
	this.form = form;
    this.success = false;
    ...
    console.log(Ajax.url+"?t="+encodeURIComponent(me.task)+"&d="+encodeURIComponent(JSON.stringify(data))<?php if(isset($_SESSION['CSRF_TOKEN']))echo "+'&csrf=".htmlspecialchars($_SESSION['CSRF_TOKEN'])."'";?>);
    $.ajax({
		url:Ajax.url+"?t="+encodeURIComponent(me.task)+"&d="+encodeURIComponent(JSON.stringify(data))<?php if(isset($_SESSION['CSRF_TOKEN']))echo "+'&csrf=".htmlspecialchars($_SESSION['CSRF_TOKEN'])."'";?>,
		data:form,
        cache:false,
        contentType:false,
        processData:false,
		...
		}else console.log("Ajax failure", d);
